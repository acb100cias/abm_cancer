...
to setup-globals
  set infection-chance 50    ;;hay que revisar la probabilidad de infección en la literatura
  ...
  set vaccine-rate random-float 1
  set vaccine-type vaccine
  set time 0
  set recruitment-rate random-float 1
  set natural-death-rate random-float 1
  set cc-death-rate 0.12
  set year 0
end
...
to setup-people
  ...
  ask women [set shape "person righty"]
  create-men initial-men
  ask men [set shape "person lefty" ]
...
;      set sane? (who <  initial-people * (1 - 0.025))
      set infected? (who < (initial-people * 0.025))
      if infected?
        [ set virus-type random 4   set infection-length random-float symptoms-show ]
      assign-commitment
      assign-coupling-tendency
      assign-condom-use
      assign-test-frequency]
     ask women [set vaccinated? (who < initial-women * vaccine-rate )]
     ask women [assign-woman-color]
     ask men [assign-man-color ]
     if vaccine-type = "vac3"[ask men [set vaccinated? (who < initial-men * vaccine-rate)]]
;  create-turtles initial-men
;  ask men  [ setxy random-xcor random-ycor
;      set known? false
;      set coupled? false
;      set partner nobody
...
to assign-man-color  ;; turtle procedure
  ask men [ifelse infected?
    [ set color red - 2  ][set color green - 2]]
  ask men[if known? [ set color blue - 2] ]

end


to assign-woman-color  ;; turtle procedure
  ask women [ifelse infected?
    [ set color red - 2  ][set color green - 2]]
  ask women[if known? [ set color blue - 2] ]
...
to new-year
  if time = 52 [set time 0  set year year + 1 crt initial-people * recruitment-rate]
end
;;;
;;; GO PROCEDURES
;;;
...
to go
  ...
  ask men [ assign-man-color ]
  ask women [ assign-woman-color ]
  file-open "red_contactos_1.txt"
  ask women [ file-write who file-write partner ]
  ;export-world "mundo.jpg"
  tick
  set time time + 1
  new-year
...
to-report %vaccinated
  ifelse any? women
    [ report (count women with [vaccinated?] / count women) * 100 ]
    [ report 0 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
PENS
"Sane" 1.0 0 -10899396 true "" "plot count turtles with [not infected?]"
"Infected" 1.0 0 -2674135 true "" "plot count turtles with [known?]"
"Infectious" 1.0 0 -13345367 true "" "plot count turtles with [infected?] - count turtles with [known?]"
"Vaccinated" 1.0 2 -7500403 true "plot count women with [vaccinated?]" "plot count women with [vaccinated?]"
